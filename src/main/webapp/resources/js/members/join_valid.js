...
document.querySelector(".join-form").addEventListener("submit", function(event) {
	...
	// 아이디 중복 검사 버튼 클릭 여부
  	var isDuplicateButtonClicked = false;
	...
	const checkButton = document.getElementById('check-duplicate-button');
	  ...
	  checkDuplicateID();
	  isDuplicateButtonClicked = true; // 중복 검사 버튼이 클릭되었음을 표시
	 });



  
    // 1-1. memberID: 영문/숫자 4~20자
	...
	// 1-2. memberID 중복 여부 확인
	if (isDuplicate) {
  	  displayErrorMessage("memberID-alert", "중복된 아이디입니다. 다른 아이디를 입력해주세요.");
  	  event.preventDefault(); // 폼 제출 기본 동작 막기
  	  return;
	}
...
window.addEventListener('DOMContentLoaded', function() {
	...
	console.log("중복확인 체크여부 : " + isDuplicateButtonClicked);
	console.log("아이디 중복됨? : " + isDuplicate);
});
